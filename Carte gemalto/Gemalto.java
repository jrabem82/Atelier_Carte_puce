...
public class Gemalto {
	...
	public boolean setPin(int numPin, byte[] oldPin, byte[] newPin) {
		/*
		 * modifier le code pin numero numePin
		 * valeur de retour : vrai si l'operation a reussi, faux sinon
		 */
	...
	public boolean emulateUserMode() {
		/*
		 * passer la carte en mode simulation utilisateur
		 * valeur de retour : vrai si l'operation a reussi, faux sinon
		 */
		// if (checkPin(0, pin0)) {
	...
	public boolean checkPin(int numPin, byte[] pin) {
		/*
		 * tester si le pin numero numPin est correct
		 * valeur de retour : vrai si l'operation a reussi, faux sinon
		 */
		ResponseAPDU r;
		byte p2;
	...
	public boolean initCard(String id, byte[] pin1, String aes) {
		 ...
		 * mettre l'id dans user2
		 * modifier le code pin 1
		 * mettre la clé AES dans user 1
		 * mettre aac pour proteger user area 1
		 * passer la carte en mode emulateur user
		 * valeur de retour : vrai si l'operation a reussi, faux sinon
		 */
		ResponseAPDU r;
		// enter PIN 0
	...
	public String readIdFromCard() {
		/*
		 * recuperer l'id de l'utilisateur dans area user 2
		 * valeur de retour : id utilisateur si l'operation a reussi, null sinon
		 */
		ResponseAPDU r;
		...
		if (r.getSW() == responseOk) {
			...
			if (r.getSW() == responseOk) {
				showLog(LOG, "readIdFromCard : lecture id ok");
	...
	public String readAesFromCard(byte[] pin1) {
		/*
		 * recuperer l'aes de l'utilisateur dans area user 1
		 * valeur de retour : ase utilisateur si l'operation a reussi, null sinon
		 */
		ResponseAPDU r;
		if (checkPin(1, pin1)) {
	...
	public byte[] format_octect(byte[] data) {// pour avoir un tableau de longueur multiple de 4
		/*
		 * formatter un tableau qlq pour avoir nu longueur multiple de 0x04, remplir le reste par du blanc
		 * valeur de retour : , tableau de byte
		 */
